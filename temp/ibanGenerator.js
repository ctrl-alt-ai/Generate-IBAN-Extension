...
export class IBANGenerationError extends Error {
    constructor(message, countryCode) {
        super(message);
...
function validateCountryCode(countryCode) {
    ...
    if (!isValidCountryCode(countryCode.toUpperCase())) {
        throw new IBANGenerationError(`Country code '${countryCode}' is not supported. Supported codes: ${Object.keys(countries).join(', ')}`, countryCode);
    }
}
/**
...
function generateBankCode(countryCode) {
    ...
    switch (countryCode) {
        case 'NL':
            ...
            return generateRandomAlphanumericString(4).replace(/\d/g, () => String.fromCharCode(65 + getSecureRandomInt(0, 25)));
        case 'DE':
        case 'ES':
        case 'BE':
        case 'CH':
        ...
        case 'GB':
            ...
            return generateRandomAlphanumericString(4).replace(/\d/g, () => String.fromCharCode(65 + getSecureRandomInt(0, 25))) + generateRandomNumericString(6);
        case 'IT':
...
function calculateCheckDigits(countryCode, bankCode, accountNumber) {
    try {
        ...
        throw new IBANGenerationError(`Failed to calculate check digits: ${error instanceof Error ? error.message : 'Unknown error'}`, countryCode);
    }
}
/**
 * Validates the generated IBAN by checking its length and structure
 */
function validateGeneratedIBAN(iban, countryCode) {
    const config = countries[countryCode];
    if (iban.length !== config.length) {
        throw new IBANGenerationError(`Generated IBAN has incorrect length. Expected ${config.length}, got ${iban.length}`, countryCode);
    }
    if (!iban.startsWith(countryCode)) {
        throw new IBANGenerationError(`Generated IBAN does not start with country code ${countryCode}`, countryCode);
    }
    // Verify check digits are numeric
    const checkDigits = iban.substring(2, 4);
    if (!/^\d{2}$/.test(checkDigits)) {
        throw new IBANGenerationError(`Invalid check digits: ${checkDigits}`, countryCode);
    }
}
/**
...
export function generateIBAN(countryCode) {
    ...
    try {
        ...
        throw new IBANGenerationError(`Unexpected error during IBAN generation: ${error instanceof Error ? error.message : 'Unknown error'}`, normalizedCountryCode);
    }
}
/**
